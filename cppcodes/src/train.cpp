...
//            std::cout << "imgs: " << imgs.sizes() << "target: " << target.sizes() << std::endl;
            ...
            {   // {}是来限定 set_autocast_enabled（） 的作用域范围
                ...
                std::tie(loss, loss_items) = compute_loss(pred, pred_mask, target, pred_mask);
...
)
{
    ...
    // create dataloader
    std::cout << ColorString("Train Datasets: ", "Info") << train_path << std::endl;
    auto [train_dataloaer, num_images] = create_dataloader_segment(train_path, imgsz, nc, batch_size, gs,
                                                            opt, hyp, augment, pad, false, false, 4); 
    
    std::cout << ColorString("Test Datasets: ", "Info") << val_path << std::endl;
    auto [val_dataloader, num_images_val] = create_dataloader_segment(val_path, imgsz, nc, batch_size * 2, gs,
                                                            opt, hyp, augment, pad, true,  false, 4);
    std::cout << " create dataloader over ...." << std::endl;
    auto model_modules = model->last_module;
...
//            std::cout << "imgs: " << imgs.sizes() << "target: " << target.sizes() << std::endl;
            ...
            {   // {}是来限定 set_autocast_enabled（） 的作用域范围
                at::autocast::set_autocast_enabled(torch::kCUDA, true);
                auto [zeros, pred, pred_mask] = model->forward(imgs);
                // std::cout << "model->forward return: ";
                // for(int i = 0; i < pred.size(); i++)
                //     std::cout << " " << pred[i].sizes();
                // std::cout << " " << pred_mask.sizes() << std::endl;
                target = target.to(device);
                masks = masks.to(device).to(torch::kFloat);
                std::tie(loss, loss_items) = compute_loss(pred, pred_mask, target, masks);
                if(std::get<bool>(opt["quad"]))
                    loss *= 4.f;
            }
            ...
            ss << " o: " << std::to_string(mloss[1].item().toFloat());
            ss << " c: " << std::to_string(mloss[2].item().toFloat());
            ss << " s: " << std::to_string(mloss[3].item().toFloat());
            ss << " t: " << std::to_string(target.size(0));
        ...
        */
        // 像coco.yaml数据集非常大，每轮训练都超过50分钟以上，如果只在最后一次保存，可能丢失训练时间太多了, 强制每次保存
        bool save_flag = num_images > 1000 ? true : false;
        if(false == save_flag)
        {
            int save_period = std::get<int>(opt["save_period"]);
